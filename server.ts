...
async function saveToMongoAll(data: typeof INITIAL_DATA) {
  ...
  try {
    ...
    console.log("[MongoDB] All collections synced successfully in background.");
  } catch (err) {
...
function saveStoreData(data: typeof INITIAL_DATA) {
  storeInMemory = data;
  try {
  ...
  // Push to MongoDB in background asynchronously
...
async function startServer() {
  ...
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      } else if (current.companyDetails) {
      ...
      } else if (current.projects && current.projects.length > 0) {
      ...
      } else if (current.lifestyleAmenities && current.lifestyleAmenities.length > 0) {
      ...
      } else if (current.testimonials && current.testimonials.length > 0) {
      ...
      } else if (current.galleryItems && current.galleryItems.length > 0) {
      ...
      } else if (current.inquiries && current.inquiries.length > 0) {
      ...
      } else if (current.recentActivities && current.recentActivities.length > 0) {
