...
import { CompanyDetails, Project, LifestyleAmenity, Testimonial, GalleryItem, Inquiry, RecentActivity, SeoSettings, DownloadCount } from "./models.js";
...
async function saveToMongoAll(data) {
    ...
    try {
        ...
        if (data.lifestyleAmenities && data.lifestyleAmenities.length > 0) {
            const cleanAmenities = data.lifestyleAmenities.map((a) => {
                const ca = { ...a };
                delete ca._id;
                delete ca.__v;
                return ca;
        ...
        if (data.testimonials && data.testimonials.length > 0) {
            const cleanTestimonials = data.testimonials.map((t) => {
                const ct = { ...t };
                delete ct._id;
                delete ct.__v;
                return ct;
        ...
        if (data.galleryItems && data.galleryItems.length > 0) {
            const cleanGallery = data.galleryItems.map((g) => {
                const cg = { ...g };
                delete cg._id;
                delete cg.__v;
                return cg;
        ...
        if (data.inquiries && data.inquiries.length > 0) {
            const cleanInquiries = data.inquiries.map((i) => {
                const ci = { ...i };
                delete ci._id;
                delete ci.__v;
                return ci;
        ...
        if (data.recentActivities && data.recentActivities.length > 0) {
            const cleanActivities = data.recentActivities.map((act) => {
                const cact = { ...act };
                delete cact._id;
                delete cact.__v;
                return cact;
    ...
    try {
        ...
        if (mongoose.connection.readyState === 1) {
            ...
            if (!company && projects.length === 0) {
                ...
                if (company) {
                    const { _id, __v, createdAt, updatedAt, ...cleanCompany } = company;
                    current.companyDetails = cleanCompany;
                }
                if (projects && projects.length > 0) {
                    current.projects = projects.map(({ _id, __v, createdAt, updatedAt, ...rest }) => rest);
                }
                if (amenities && amenities.length > 0) {
                    current.lifestyleAmenities = amenities.map(({ _id, __v, createdAt, updatedAt, ...rest }) => rest);
                }
                if (testimonials && testimonials.length > 0) {
                    current.testimonials = testimonials.map(({ _id, __v, createdAt, updatedAt, ...rest }) => rest);
                }
                if (gallery && gallery.length > 0) {
                    current.galleryItems = gallery.map(({ _id, __v, createdAt, updatedAt, ...rest }) => rest);
                }
                if (inquiries && inquiries.length > 0) {
                    current.inquiries = inquiries.map(({ _id, __v, createdAt, updatedAt, ...rest }) => rest);
                }
                if (activities && activities.length > 0) {
                    current.recentActivities = activities.map(({ _id, __v, createdAt, updatedAt, ...rest }) => rest);
                }
                if (seo) {
                    const { _id, __v, createdAt, updatedAt, ...cleanSeo } = seo;
...
app.put("/api/admin/testimonials/:id", (req, res) => {
    try {
        ...
        if (name !== undefined)
            store.testimonials[index].name = name;
        if (role !== undefined)
            store.testimonials[index].role = role;
        if (content !== undefined)
            store.testimonials[index].content = content;
        if (rating !== undefined)
            store.testimonials[index].rating = Number(rating);
        if (avatar !== undefined)
            store.testimonials[index].avatar = avatar;
        if (projectPurchased !== undefined)
            store.testimonials[index].projectPurchased = projectPurchased;
        if (hidden !== undefined)
            store.testimonials[index].hidden = !!hidden;
        saveStoreData(store);
...
app.delete("/api/admin/gallery/:id", (req, res) => {
    try {
        const { id } = req.params;
        const store = getStoreData();
        store.galleryItems = store.galleryItems.filter((g) => g.id !== id);
        saveStoreData(store);
        res.json({ success: true });
    }
    catch (err) {
...
app.put("/api/admin/gallery/:id", (req, res) => {
    try {
        ...
        const { title, category, image, projectId } = req.body;
        const store = getStoreData();
        const index = store.galleryItems.findIndex((g) => g.id === id);
        if (index === -1) {
            return res.status(404).json({ error: "Gallery item not found." });
        }
        if (title !== undefined)
            store.galleryItems[index].title = title;
        if (category !== undefined)
            store.galleryItems[index].category = category;
        if (image !== undefined)
            store.galleryItems[index].image = image;
        if (projectId !== undefined)
            store.galleryItems[index].projectId = projectId;
        saveStoreData(store);
